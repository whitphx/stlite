...
from .httputil import parse_body_arguments, HTTPFile
...
class UploadFileRequestHandler(RequestHandler):
    def __init__(self, file_mgr: UploadedFileManager, is_active_session: Callable[[str], bool]) -> None:
    ...
    def post(self, request: Request, **kwargs) -> Tuple[int, dict, bytes]:
        ...
        if len(uploaded_files) != 1:
            return 400, {}, f"Expected 1 file, but got {len(uploaded_files)}".encode("utf-8")
