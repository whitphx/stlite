...
from stlite_cli._runtime_version import RUNTIME_VERSION as DEFAULT_RUNTIME_VERSION
from stlite_cli.app_data import build_app_data
from stlite_cli.export_html import export_as_html

# `_runtime_version.py` is generated by `make cli-py-runtime-version` from
# `packages/browser/package.json`. Pinning the default to the bundled
# browser version keeps the cli's `stlite html` output in lockstep with
# the workspace — same approach as the JS bin (see scripts/build.js).
# Override per-invocation with `--runtime-version`.
