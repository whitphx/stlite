...
You can mount multiple independent Stlite apps on the same page by calling `mount()` multiple times with different container elements. Each app runs in its own Web Worker with isolated session state.
