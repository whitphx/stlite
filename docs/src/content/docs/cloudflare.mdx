...
A live demo of the [`hello` sample](https://github.com/whitphx/stlite/tree/main/packages/cloudflare/samples/hello) built this way runs at [https://stlite-cloudflare.whitphx.workers.dev/](https://stlite-cloudflare.whitphx.workers.dev/).
...
This produces a deployable Worker directory at `./dist` (default) containing `wrangler.jsonc`, `src/entry.py`, `python_modules/` (your app, the Stlite runtime, and the vendored Python dependency tree), and `assets/` (the Streamlit frontend).
...
| `--asset-runtime`       | Ship the Python runtime as static assets instead of bundling it into the script (see below).   |
...
### Where the runtime ships: `--asset-runtime`
