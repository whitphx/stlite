...
- Networking libraries are not working in general, but _stlite_ uses [`koenvo/pyodide-http`](https://github.com/koenvo/pyodide-http)'s patch to make [these packages and methods](https://github.com/koenvo/pyodide-http?tab=readme-ov-file#supported-packages) work, including `requests` and `urllib`. [`pyodide.http.pyfetch()`](https://pyodide.org/en/stable/usage/api/python-api/http.html#pyodide.http.pyfetch) and [`pyodide.http.open_url()`](https://pyodide.org/en/stable/usage/api/python-api/http.html#pyodide.http.open_url) are also options, which are provided as standard HTTP accessors by Pyodide, as [this document says](https://pyodide.org/en/stable/usage/faq.html#how-can-i-load-external-files-in-pyodide). To use `pyodide.http.pyfetch()`, see also the following section about top-level await.
