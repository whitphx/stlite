...
TEXT_EXTS = frozenset(
    {".py", ".txt", ".csv", ".md", ".json", ".yml", ".yaml", ".toml"}
)
...
def build_app_data(
    *,
    ...
    requirements = (
        read_requirements(requirements_path) if requirements_path else []
    )
...
def collect_files(root: Path) -> dict[str, File]:
    rel_paths: list[str] = [
        abs_path.relative_to(root).as_posix() for abs_path in walk(root)
    ]
    ...
    for rel in rel_paths:
        ...
        if abs_path.suffix.lower() in TEXT_EXTS:
            out[rel] = File(text=abs_path.read_text(encoding="utf-8"))
        else:
