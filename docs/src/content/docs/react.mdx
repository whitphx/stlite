...
import sharedWorkerAppRaw from '../../../../packages/react/demos/shared-worker/App.tsx?raw';

export const basicApp = transformReactDemoImports(basicAppRaw);
export const multiApp = transformReactDemoImports(multiAppRaw);
export const sharedWorkerApp = transformReactDemoImports(sharedWorkerAppRaw);
...
<Code code={sharedWorkerApp} lang="tsx" title="SharedWorkerApp.tsx" />

:::note
- SharedWorker mode shares the Python environment and installed packages across all apps
- Each app still has its own home directory (`/home/pyodide/<app-id>`)
- SharedWorker is not supported in all browsers (e.g., Chrome on Android, older Safari versions)
- If SharedWorker is not supported, the app will automatically fall back to a dedicated worker
:::
