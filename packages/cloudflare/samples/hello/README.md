...
It builds with `--plain-worker` (not the default Durable Object mode): the
demo is read-only — no `st.file_uploader` — and the Animation page needs more
memory than a single 128 MB Durable Object instance can hold once every
page's imports share it.
