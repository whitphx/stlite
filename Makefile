...
# Generate `_runtime_version.py` so `stlite html`'s default
# `--runtime-version` is the bundled `@stlite/browser` version. The JS bin
# achieves this via esbuild `define:` (see packages/cli/scripts/build.js);
# this target is the Python-side mirror. Output is gitignored — must be
# generated before the Python wheel is built or the parity test runs.
.PHONY: cli-py-runtime-version
cli-py-runtime-version: $(cli-py-runtime-version)
$(cli-py-runtime-version): packages/browser/package.json
	@version=$$(node -p "require('./packages/browser/package.json').version") && \
		printf '# Auto-generated by `make cli-py-runtime-version` from packages/browser/package.json. Do not edit.\nRUNTIME_VERSION = "%s"\n' "$$version" > $@
...
$(streamlit-frontend-lib): $(node_modules) $(streamlit_proto) $(shell find streamlit/frontend/connection streamlit/frontend/utils streamlit/frontend/component-v2-lib \
  -type f ! -path '*/dist/*' \
  \( -name '*.ts' -o -name '*.tsx' -o -name 'package.json' -o -name 'tsconfig.json' \))
	yarn workspaces foreach --recursive --from '{@streamlit/connection,@streamlit/utils,@streamlit/component-v2-lib}' --topological run build
	@mkdir -p $(dir $@)
	@touch $@
