...
<img
  src={stliteDesktopBanner.src}
  alt="Stlite Desktop Banner"
...
import cliPkg from "@stlite/cli/package.json";

export const packageJsonWithVersion = packageJson
  .replace("{{VERSION}}", pkg.version)
  .replace("{{CLI_VERSION}}", cliPkg.version);
...
This invokes [`stlite desktop`](/cli/#stlite-desktop-path) from [`@stlite/cli`](/cli/), which reads the [`stlite.desktop`](#configuration) section of `package.json`, vendors the Pyodide runtime, and writes the desktop artifact tree into `./build`.
